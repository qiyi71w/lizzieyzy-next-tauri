...
| Editable current-game Save | `editable_workspace_roundtrip` plus `current_game_save_write_failure` (directory-as-file) and `save-as-dialog` coverage for cancel/chosen/denied/redirected dialog outcomes. Redirected user-profile paths are write failures and are not adopted. | Ticket 08 Windows native: cancel and happy-path Save As/reopen passed on `8c749a6` (PID 75084). Case 6 ACL redirect remains native re-acceptance of ticket 08's Case 6 shape after ticket 09. |
...
| SGF-06 | Partial | Ticket 07 `editable_workspace_roundtrip` and `current_game_save_write_failure`, plus ticket 09 `save-as-dialog` redirect/deny/cancel/chosen coverage. | Case 4 cancel and Case 5 Save As/reopen met the happy-path acceptance text. Case 6 still needs native re-acceptance after ticket 09. | Native re-acceptance of ticket 08 Case 6. |
