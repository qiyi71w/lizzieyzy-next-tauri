...
The task panel previews count/endpoints, selects single-stage, all-position two-stage, or swing-selected two-stage execution, edits their independent conditions and swing criteria, and shows immutable active conditions, stage-local progress and ending causes. Explicit Start persists `taskSingleStageConditions`, `taskOverviewConditions`, `taskDeepConditions`, `taskSwingOverviewConditions`, `taskSwingDeepConditions` and `taskSwingCriteria` through the existing serialized atomic preferences transaction before admission. Legacy `taskConditions` becomes the unchanged single-stage preset and seeds a separate all-position deep preset whose total-visits value is floored to 500; unrelated preferences remain intact. All-position overview defaults to 32 total visits. Missing all-position deep presets derive total visits from the existing single-stage/default value with a floor of 500; a supported stored deep preset at or above 500 is retained. Explicit task limits support 1–4294967295 independently of the finite selected-node preference. Failed writes preserve durable preferences and current work; later successful writes affect future tasks only. Ctrl+B uses a one-visit first-child single-stage preset without changing durable task presets; Ctrl+Shift+B starts the durable all-position two-stage preset. Both use registry focus suppression. Existing finite selected-node and contextual Space controls remain independent. Task state is never serialized into SGF or recovery.
...
`pause_analysis_task` seals the task's Job under the current-game holder lock and requests target cancellation. `analysis_task_snapshot` exposes `pausing` until target-final cleanup, then `paused`; a control ACK alone cannot release cleanup. The paused task retains the whole-game reservation, frozen worklist, stage and stage-local completion sets. `continue_analysis_task` rechecks semantic generation, Run and departure admission, creates a fresh Job identity, skips positions completed in the current stage and restarts the interrupted position with that stage's full immutable budget. A completed overview resumes at deep without repeating overview work. It neither restarts the Run nor changes selected-node intent/error/departure holds. Pause/Cancel and stage/next-position query delivery are serialized, preventing a cancellation from being overtaken by a queued submission. Cancel and confirmed departure terminate a paused task; cancellation delivery/deadline failure follows bounded Run cleanup and aborts an in-flight departure. The frontend provides Pause/Continue alongside Start/Cancel and fences delayed pre-Pause events without stopping independent selected-node work.

## Current-game Analysis Task Contract

`ANA-16` is Accepted after repository verification and exact-candidate Windows real-KataGo acceptance at `5e593537f702af0c651a2c3bf0f0fd77758052e2`. The [approved specification](../.scratch/current-game-analysis-tasks/spec.md) and [completed tracker](../.scratch/current-game-analysis-tasks/tracker.md) record scope, budgets, Pause/Continue, both two-stage strategies and their acceptance evidence. Operational Closeout is complete with no follow-up candidates; independent R5 evidence remains with its owners.

The SGF domain resolves an exact NodePath worklist; the Rust current-game owner admits it and fences result attachment against position/tree changes. The engine manager owns one session-only Analysis Task in the existing whole-game lane, independently of the selected-node lane. Task identity includes the semantic game, immutable scope/stage conditions, Analysis Context and Foreground Engine Run. Navigation, personal comments and ordinary Save preserve identity; semantic/tree changes or a new Run require an explicit new task.

Pause fences publication and cancels the active target before becoming Paused. Continue retains only that task's current-stage completed set; incomplete work restarts its stage budget. Existing SGF visit counts do not establish task completion. Overview summaries remain task-owned even when continuous or deep analysis replaces visible SGF results. Swing-selected tasks preview required Supporting Positions, compare before/after results from a common perspective, and freeze their deduplicated Deep set after Overview; later continuous or Deep results cannot reshape it.

Tasks reuse the target-final cancellation, Run-failure, coherent Save, safe-departure and last-successful recovery contracts. Task state is not recovered after restart. Wire changes start in app-model and flow through mirrored TypeScript types and API wrappers; the gateway remains an adapter.
