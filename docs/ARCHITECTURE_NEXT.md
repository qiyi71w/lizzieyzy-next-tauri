...
## Explicit Single-stage Analysis Tasks
...
The manager retains one session-only `AnalysisTaskDto`, distinct from its underlying Job, with frozen scope, Run, semantic generation, single-stage conditions, requested paths and completed paths. Only legitimate final results reaching the total-visit budget count as completion. Enabled time/leading-candidate conditions reject until their owning ticket lands. `analysis_task_snapshot` exposes queued/searching and retained completed/cancelled/failed/invalidated states. Cancel seals publication immediately while the existing target-final cleanup completes. Semantic edits, Stop/Restart and successful Run replacement invalidate; navigation, comments, ordinary Save and failed switches retaining the original Run preserve work.

The task panel previews count/endpoints, starts a validated total-visit request, and shows real progress and terminal reasons. Explicit preset writes reuse atomic app preferences and the retained default visits (1–1,000,000); failure starts nothing. Ctrl+B uses a one-visit first-child quick preset through the action registry with editable-focus suppression. Direct task conditions admit positive `u32` total visits. Existing finite selected-node and contextual Space controls remain independent. Task state is never serialized into SGF or recovery. This slice has repository/controllable-engine evidence; ANA-16 remains Missing pending the complete batch and ticket 07 native acceptance.
