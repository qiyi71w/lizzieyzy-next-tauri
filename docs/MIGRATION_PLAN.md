...
- UI automation for the primary desktop smoke flow. Ticket 08 recorded a human-guided Windows no-engine run on `8c749a6`: launch/open/navigate/edit/save/reopen passed; ACL-denied Save As failed. Ticket 09 adds Gateway `save-as-dialog` coverage for that Case 6 redirect shape; native re-acceptance is still required.
