...
Ticket 26 later split the unverified bundle: frozen Yike Personal-category discovery is Deferred `PROV-06`, while provider-supported account authorization, account-authorized live-room reads, and native human play are Deferred Next redesign `PROV-07`. Java proves no Yike login, private-room type, or native Yike move API. Neither item expands `PROV-01` or `PROV-03`; [Ticket 26](26-decide-yike-personal-auth-reads.md) owns their complete contracts.
...
| `CAP-06-YIKE-LIVE-CENTER` browse live games then sync | Split Next redesign / Deferred | The provider center lists public `recommend` and `local` modes as preview (`PROV-01`). Import stays `PROV-01`; Start sync and Play & Sync stay `PROV-03`. Frozen Personal-category discovery is separate Deferred `PROV-06`; provider-authenticated read and native human play are separate Deferred Next redesign `PROV-07`. |
...
| `CAP-06-SYNC-SETTINGS` live/readboard toggles | Route to owner items | Sync intervals, locators, recents, retries, jump-to-last, mute-during-sync, provider-account references, and readboard always-sync/focus belong to their `PROV-01`–`PROV-07` and `READ-02` behavior owners. They are not duplicate generic `PREF-01` capabilities. Persistence uses `PREF-01` as the durable-preference mechanism only; provider secrets use the System Credential Store only where `PROV-07` permits them. |
...
- `PROV-03` — **Yike Ongoing Synchronization**: new item, not combined with `PROV-01`. Explicit Start sync applies the ongoing-synchronization model to a public Yike room. **Play & Sync** is the dual-channel redesign: the system browser owns login, cookies, and webpage move submission for that room; Next starts the matching public read-only sync and uses only its signed read path; Next does not read, copy, or log browser authentication. Stopping Next sync leaves the browser open. Native authenticated human play belongs only to Deferred `PROV-07`; embedded web/hall hosting is not this item.
...
- `PROV-06` — **Yike Personal-category Discovery**: Deferred. Own only the frozen Personal category, pagination/filtering, result presentation, and handoff to `PROV-01`/`PROV-03`; keep Recommend as default and do not persist category/page. Guest semantics require live evidence before promotion.
- `PROV-07` — **Yike Authenticated Read and Play**: Deferred Next redesign with no frozen implementation evidence. Own one formally authorized Yike account, account-authorized live-room reads, and provider-authoritative human Move/Pass/Resign through the sole Match Session. Ticket 26 fixes its credential, admission, pending/confirm, failure, recovery, dependency, and evidence contracts.
...
**Credentials.** Public `PROV-03` browser authentication stays in the system browser, and public Yike signed reads persist no user secrets, browser cookies, auth material, or signing values in settings or logs. Deferred `PROV-07` is the only Yike account exception: it accepts only provider-supported authorization, stores provider-issued secrets in the System Credential Store, and persists only a non-secret account reference.
...
**Retry.** Retry only idempotent transient reads, at most three retries after the initial attempt. Do not retry auth failures, not-found, malformed or invalid provider content, or user cancellation. `PROV-07` Move/Pass/Resign writes are never automatically retried.

**Recovery.** Ignore stale results and preserve the last-good board on failure. Stop or cancel leaves an editable current game for read-only provider sessions. `PROV-07` instead keeps an unresolved write in visible Error/Reconcile under the sole Match Session until authoritative readback or an explicitly warned Stop. No provider or Match Session is recovered by `APP-04`.

**Privacy.** Persist only bounded, clearable, non-secret recents (URLs, usernames, UIDs, room locators) and non-secret credential references. Clearing provider/search history is owned by these items, not by a broad reset. Secrets, auth headers, provider responses, and active sessions are never preference data.
...
**Live evidence.** Separate and per mode. Required for numbered R10 work: Yike public categories; every accepted public Yike room URL family; Play & Sync handoff (system-browser room, web login/move, Next public sync); Fox and Tencent lookup modes and pagination; upstream failure and latency; readboard readiness, sync, disconnect, and restart. Deferred `PROV-06`, `PROV-07`, `PROV-05`, and `PUB-01` do not need current live evidence; Ticket 26 records their promotion evidence.

Existing Partial items remain Partial until both their repository evidence and in-scope live evidence pass. Ticket 15 owns phase ordering; Ticket 16 owns inventory, matrix, and roadmap traceability. Ticket 26 supersedes this ticket's combined personal/private/auth assumption without expanding `PROV-01` or `PROV-03`. Deferred acceptance is recorded now so later work cannot silently broaden it.
